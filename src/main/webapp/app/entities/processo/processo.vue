<template>
  <div>
    <h2 id="page-heading" data-cy="ProcessoHeading">
      <span v-text="$t('advappApp.processo.home.title')" id="processo-heading">Processos</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('advappApp.processo.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'ProcessoCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('advappApp.processo.home.createLabel')"> Create a new Processo </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && processos && processos.length === 0">
      <span v-text="$t('advappApp.processo.home.notFound')">No processos found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="processos">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="$t('advappApp.processo.nuj')">Nuj</span></th>
            <th scope="row"><span v-text="$t('advappApp.processo.nemroProcessoTJ')">Nemro Processo TJ</span></th>
            <th scope="row"><span v-text="$t('advappApp.processo.vara')">Vara</span></th>
            <th scope="row"><span v-text="$t('advappApp.processo.obs')">Obs</span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="processo in processos" :key="processo.id" data-cy="entityTable">
            ...
            <td>{{ processo.nuj }}</td>
            <td>{{ processo.nemroProcessoTJ }}</td>
            <td>{{ processo.vara }}</td>
            <td>{{ processo.obs }}</td>
            <td class="text-right">
              <div class="btn-group">
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="advappApp.processo.delete.question" data-cy="processoDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-processo-heading" v-text="$t('advappApp.processo.delete.question', { id: removeId })">
