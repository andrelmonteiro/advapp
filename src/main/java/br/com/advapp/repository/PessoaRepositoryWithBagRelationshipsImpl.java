...
import br.com.advapp.domain.Pessoa;
...
public class PessoaRepositoryWithBagRelationshipsImpl implements PessoaRepositoryWithBagRelationships {
    ...
    public Optional<Pessoa> fetchBagRelationships(Optional<Pessoa> pessoa) {
        return pessoa.map(this::fetchEnderecos).map(this::fetchProcessos);
    }
    ...
    public List<Pessoa> fetchBagRelationships(List<Pessoa> pessoas) {
        return Optional.of(pessoas).map(this::fetchEnderecos).map(this::fetchProcessos).orElse(Collections.emptyList());
    }
    ...
    Pessoa fetchProcessos(Pessoa result) {
        return entityManager
            .createQuery("select pessoa from Pessoa pessoa left join fetch pessoa.processos where pessoa is :pessoa", Pessoa.class)
            .setParameter("pessoa", result)
            .setHint(QueryHints.PASS_DISTINCT_THROUGH, false)
            .getSingleResult();
    }

    List<Pessoa> fetchProcessos(List<Pessoa> pessoas) {
        HashMap<Object, Integer> order = new HashMap<>();
        IntStream.range(0, pessoas.size()).forEach(index -> order.put(pessoas.get(index).getId(), index));
        List<Pessoa> result = entityManager
            .createQuery(
                "select distinct pessoa from Pessoa pessoa left join fetch pessoa.processos where pessoa in :pessoas",
                Pessoa.class
            )
            .setParameter("pessoas", pessoas)
            .setHint(QueryHints.PASS_DISTINCT_THROUGH, false)
