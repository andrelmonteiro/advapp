/* tslint:disable max-line-length */
import { shallowMount, createLocalVue, Wrapper } from '@vue/test-utils';
import sinon, { SinonStubbedInstance } from 'sinon';
import VueRouter from 'vue-router';

import * as config from '@/shared/config/config';
import ProcessoDetailComponent from '@/entities/processo/processo-details.vue';
import ProcessoClass from '@/entities/processo/processo-details.component';
import ProcessoService from '@/entities/processo/processo.service';
import router from '@/router';
import AlertService from '@/shared/alert/alert.service';

const localVue = createLocalVue();
localVue.use(VueRouter);

config.initVueApp(localVue);
const i18n = config.initI18N(localVue);
const store = config.initVueXStore(localVue);
localVue.component('font-awesome-icon', {});
localVue.component('router-link', {});

describe('Component Tests', () => {
  describe('Processo Management Detail Component', () => {
    let wrapper: Wrapper<ProcessoClass>;
    let comp: ProcessoClass;
    let processoServiceStub: SinonStubbedInstance<ProcessoService>;

    beforeEach(() => {
      processoServiceStub = sinon.createStubInstance<ProcessoService>(ProcessoService);

      wrapper = shallowMount<ProcessoClass>(ProcessoDetailComponent, {
        store,
        i18n,
        localVue,
        router,
        provide: { processoService: () => processoServiceStub, alertService: () => new AlertService() },
      });
      comp = wrapper.vm;
    });

    describe('OnInit', () => {
      it('Should call load all on init', async () => {
        // GIVEN
        const foundProcesso = { id: 123 };
        processoServiceStub.find.resolves(foundProcesso);

        // WHEN
        comp.retrieveProcesso(123);
        await comp.$nextTick();

        // THEN
        expect(comp.processo).toBe(foundProcesso);
      });
    });

    describe('Before route enter', () => {
      it('Should retrieve data', async () => {
        // GIVEN
        const foundProcesso = { id: 123 };
        processoServiceStub.find.resolves(foundProcesso);

        // WHEN
        comp.beforeRouteEnter({ params: { processoId: 123 } }, null, cb => cb(comp));
        await comp.$nextTick();

        // THEN
        expect(comp.processo).toBe(foundProcesso);
      });
    });

    describe('Previous state', () => {
      it('Should go previous state', async () => {
        comp.previousState();
        await comp.$nextTick();

        expect(comp.$router.currentRoute.fullPath).toContain('/');
      });
    });
  });
});
